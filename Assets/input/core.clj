(ns input.core
  ...
  (:import [UnityEngine Input Vector2 Camera]))
...
(defn update-input-state!
  ...
  {:movement (.normalized (lin/v2 (Input/GetAxisRaw (input-axes :horizontal))
   ...
   :aim (.normalized (lin/v2- (let [pos (.ScreenToWorldPoint Camera/main Input/mousePosition)]
                        (lin/v2 (.x pos) (.y pos)))
...
(defn push-input!
  ...
  (let [other (arc/state this :output-obj)]
    (when other
      (arc/update-state other :input update-input-state! (.position (.transform this))))))
